...
#define NUM_CHANNELS       1 //vorher: 3
...
public:
  ...
  LcdType () :  backlightalarm(*this), _present(false), first(true), backlightOnTime(10), lcd(LCD_ADDRESS, 16, 2){}	// Konstruktor setzt LCD nicht präsent, erter init, 10s Baacklight, Adresse , 16 ZEichen, zwei Zeilen
  virtual ~LcdType () {}

  void displayValues(_currentSensor *cs) {	// zeige Messwerte an Stromsensor cs
    if (_present == true) {
  ...
  void showCondition(uint8_t conditionType, uint8_t channel) {		// Schreibe den Pfeil
    lcd.setCursor(channel * 6 + 3, 0);								// Setze Cursor Kanal * 6 + 3 Zeichen, 0 (Spalte, Zeile)
    if (conditionType == ct_above) lcd.write(byte(0));				// Kanal 0 * 6 + 3 = 3
    if (conditionType == ct_below) lcd.write(byte(1));				// 
    if (conditionType == ct_none) lcd.print("*");
    if (conditionType == ct_disabled) lcd.print(" ");
  }

  void initLCD(uint8_t *serial) {						// Display initialisieren
    Wire.begin();
    Wire.beginTransmission(LCD_ADDRESS);
    if (Wire.endTransmission() == 0) {
      _present = true;
      lcd.init();
      lcd.createChar(0, arrowUp);						// Pfeil oben in byte 0 schreiben
      // create a new character
      lcd.createChar(1, arrowDown);						// Pfeil unten in byte 1 schreiben
      lcd.backlight();
      lcd.setCursor(0, 0);
...
//         cumulatedCurrentValues[2] += cs[2].current;
         ...
         if (measureCount >= dev.txInterval) {
           ...
           for (uint8_t ch = 0; ch < NUM_CHANNELS; ch++)
             cumulatedCurrentValues[ch] /= measureCount;			// Mittelwert berechnen
